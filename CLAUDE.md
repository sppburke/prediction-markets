...
`prediction-edge` is a Rust 1.95.0 / Rust 2024 trading and research system for Polymarket and Kalshi. The workspace is production-wired: `crates/`, `Cargo.toml`, `Cargo.lock`, `rust-toolchain.toml`, `deny.toml`, and `.github/workflows/ci.yml` are in place and the full acceptance gate is green. A live paper copy-trader (`pe-service`) is operational, implementing the Winner-Follow strategy with crash-safe state (`pe-paper-state`), P&L tracking and resolution ingestion (`pe-paper-pnl`), a data-pipeline bootstrap (`pe-bootstrap`), and walk-forward backtesting (`pe-backtest`). Wallet ranking is the standalone buy-and-hold pipeline (`scripts/rank_and_push.sh`; TTR 48h + MinTRL-20 since the 2026-07-03 run28 cutover — the "72hr" in script names is historical). Subsequent work adds live execution and additional strategies per `docs/17-RUST-IMPLEMENTATION-ROADMAP.md`.
