...
async fn pre_start_boot_membership_case(empty: bool) {
    ...
    let router = Router::new().fallback(get(respond)).with_state(source);
...
async fn fresh_pre_start_boot_pins_batch_and_brackets_active_main_survivors() {
    pre_start_boot_membership_case(false).await;
}

#[tokio::test]
async fn fresh_pre_start_boot_refuses_empty_active_main_selection() {
    pre_start_boot_membership_case(true).await;
}
