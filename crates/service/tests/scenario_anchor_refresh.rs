...
async fn deferred_refresh_continues_the_poller() {
    ...
    responses
        ...
        .unwrap()
        .insert(0, b"[]".to_vec());
    responses.get_mut(&position_url(wallet, PositionPartition::NotRedeemable)).unwrap().insert(0,
        serde_json::to_vec(&vec![json!({
            "proxyWallet": wallet, "asset": "unmapped", "conditionId": "condition", "size": "1", "outcomeIndex": 0
        })]).unwrap());
    responses
        .get_mut(&position_url(wallet, PositionPartition::Redeemable))
        .unwrap()
        .insert(0, b"[]".to_vec());
    let ordinary_page = stable_bracket_responses(&[wallet])[&activity_url(wallet)][0].clone();
