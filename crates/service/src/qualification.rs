...
use pe_position_ledger::{
    AppliedEffect, IdentityCorrection, LedgerMutation, PositionLedger, SecondVerdict,
...
fn decision_observation_from_source(
    ...
    for page in continuation.page_occurrences() {
        ...
        if !crate::bucket_commit::activity_page_generation(
            &observation.source_id,
            observation.schema_version,
            observation.parser_version,
            &observation.content_type,
        )
        .is_ok_and(|generation| generation.matches_continuation(continuation.version()))
...
/// Source-selected rows plus every receipt-bearing history row inside the sealed prefix.
#[derive(Debug, Default)]
pub(crate) struct SelectedDecisionRows {
    pub rows: Vec<DecisionPendingRow>,
    pub in_prefix: Vec<(pe_core_types::SourceTradeId, String)>,
}
...
fn decision_rows_from_sealed_source(
    ...
    let terminal_rows = history
        ...
        .collect::<HashMap<_, _>>();
    // V5 stream aliases are evidence in the authenticated complete read. Map their earliest
    ...
    let current_reads = if history.iter().any(|row| {
        ...
        for read in &reads {
            ...
            let receipt = read.continuation.read_commitment.ok_or_else(|| {
                QualificationError::InsufficientEvidence("binding commitment missing".to_owned())
            })?;
            ...
            for binding in commitment.bindings.into_iter().flatten() {
                    ...
                    && let Some(sequence) = source_universe.remove(&binding.stream_group_id)
                {
                    source_universe
                        ...
                        .and_modify(|first| *first = (*first).min(sequence))
                        .or_insert(sequence);
                }
            }
        }
    ...
    let complete_reads = match current_reads {
        Some(reads) => reads,
        None => complete_activity_read_scopes(&history, sealed_sequence, source)?,
    };
        ...
        if !complete_reads.iter().any(|read| {
            read.decisions.contains_key(source_trade_id)
                && read.aggregates.iter().any(|aggregate| {
                    aggregate.group_id.key() == source_trade_id
                        && aggregate.semantic_revision.as_str() == revision
                })
        }) {
            return insufficient(format!(
                "source-log trade {source_trade_id} has no reconstructable receipt-bearing decision"
    ...
    let rows = required
        ...
        .collect::<Result<Vec<_>, _>>()?;
    Ok(SelectedDecisionRows { rows, in_prefix })
}

#[derive(Debug, Clone)]
struct CompleteActivityReadScope {
    continuation: DecisionContinuationV3,
    aggregates: Vec<pe_source_polymarket_public::ActivityAggregate>,
...
fn complete_activity_read_scopes(
    ...
    for row in rows {
        ...
        let invalid_read = |error| {
            QualificationError::InsufficientEvidence(format!(
                "source-log trade {} has no reconstructable receipt-bearing decision: {error}",
                row.source_trade_id
            ))
        };
        ...
        {
            if !existing.continuation.same_complete_read(&continuation) {
                return insufficient(
            ...
            existing
                .bindings
                .verify_facts(&continuation.facts)
        ...
        let (aggregates, bindings) = continuation
            .reconstruct_complete_activity_read_with_bindings(&mut lookup)
            .map_err(invalid_read)?;
...
fn decision_keys_from_source_observations(
        ...
        for (source_trade_id, receipt) in &read.decisions {
            ...
            if read.continuation.version() != 5 {
                ...
                if observation.source_id != crate::activity_ingest::ACTIVITY_WS_SOURCE_ID
                    ...
                    || observation.parser_version
                        != pe_source_polymarket_public::ACTIVITY_PARSER_VERSION
                    || activity.wallet != read.continuation.facts.wallet
                ...
                read.continuation
                    .verify_stream_binding(source_trade_id, receipt, &mut |receipt| {
                        let observation = source.observation(receipt)?;
                        Ok::<_, QualificationError>(CompleteActivityPage {
                            payload: observation.payload,
                            observed_at: observation.observed_at,
                    ...
                    .map_err(|error| {
                        QualificationError::InsufficientEvidence(format!(
                            "decision websocket observation has the wrong source contract: {error}"
                        ))
                    })?;
            }
            first_observations
    ...
    for read in complete_reads {
        for aggregate in &read.aggregates {
...
pub(crate) fn verify_decision_continuation_facts(
    ...
    let frozen_identity = MarketOutcomeId::new(frozen.market_id.clone(), frozen.outcome_id);
    let identity_matches = match correction {
        Some(correction) => {
            raw_identity.as_ref() == Some(&correction.stamped)
                && correction.verified == frozen_identity
        }
        None => raw_identity.as_ref() == Some(&frozen_identity),
    };
...
fn retain_live_preparation_sources(
    prefix: &LogTailBinding,
    receipts: &HashSet<(EventSeq, blake3::Hash)>,
) -> Result<Vec<EventEnvelope>, QualificationError> {
    let mut source_envelopes = Vec::new();
    // The scanner visits each sequence once, in order. The prefix observer retains at most one
    // envelope per selected tuple, cloning only after membership succeeds; outer retained bytes
    // scale with selected payload bytes. Existing journal and recursive-reducer allocations are
    // outside this bound.
    if Scanner::walk_prefix(prefix, &mut |_, envelope| {
        if receipts.contains(&(envelope.seq, envelope.this_hash)) {
...
fn verify_live_preparation_posture(
    ...
    for account_id in account_ids {
        ...
        receipts.extend(crate::live_fanout::projection_source_receipts(
            &account_id,
            &events,
        ));
        account_events.push((account_id, events));
    }
    let scan = Scanner::inspect(source_log_path)?;
    let source_envelopes = retain_live_preparation_sources(&scan.verified_tail, &receipts)?;
...
mod tests {
    ...
    fn read_scope(
        ...
        continuation.page_occurrences = pages
            ...
            .collect();
        let (aggregates, bindings) = continuation
            .reconstruct_complete_activity_read_with_bindings(&mut |receipt| -> Result<
                _,
                QualificationError,
            > {
                let source = decision_source_receipt(observations, receipt)?;
                Ok(CompleteActivityPage {
                    payload: source.payload.clone(),
                    observed_at: source.observed_at.clone(),
                    received_at: source.received_at.clone(),
    ...
    #[test]
    fn complete_second_replay_dispatches_by_continuation_generation() {
        let mut rows = (0..5)
            .map(|index| {
                activity_row(
                    "market-a",
            ...
            .collect::<Vec<_>>();
        rows.push(activity_row(
            "market-b", "asset-b", "1", "0.5", "entry-b", 100,
        ));
        let payload = activity_payload(rows);
        let aggregates = parsed_aggregates(&[&payload]);
        let target = aggregates
            .iter()
            .find(|aggregate| aggregate.group_id.components().transaction_hash == "entry-b")
            .unwrap();
        let observation = activity_observation(2, &payload);
        let (page, evidence) = activity_page_pair(&observation, &payload, None, 100, 0);
        let (template, _) = single_read_fixture(2, "market-b", "BUY", false);
        let mut frozen = template.facts;
        frozen.source_trade_id = target.group_id.key().clone();
        frozen.semantic_revision = target.semantic_revision.as_str().to_owned();
        frozen.transaction_hash = "entry-b".to_owned();
        frozen.decision_inputs = serde_json::json!({"fixed_end":100,"pages":[evidence]});
        let original = DecisionContinuationV3::new(frozen, None, vec![page], None);
        let mutations = aggregates
            .iter()
            .map(|aggregate| LedgerMutation::from_activity(aggregate).unwrap())
            .collect::<Vec<_>>();
        let ledger = PositionLedger::new();
        let (_, expected) = ledger.simulate_all_or_none(&mutations).unwrap();
        for version in [4, 5] {
            let mut continuation = original.clone();
            let mut observations = BTreeMap::from([(2, observation.clone())]);
            if version == 5 {
    ...
    fn decision_keys_exclude_saturated_parent_rows() {
        ...
        let target_id = expected.group_id.key().clone();
        let read = read_scope(100, pages, [target_id], &observations);
    ...
    fn split_source_fixture() -> (DecisionContinuationV3, BTreeMap<u64, SourceObservation>) {
    ...
    fn qualification_source_verifier_excludes_saturated_parent_rows() {
        let (continuation, observations) = split_source_fixture();
        ...
        let observation = verify_decision_source_inputs(&state, &decision, &observations).unwrap();
    ...
    fn source_trade_cannot_disappear_with_every_projection_deleted() {
        ...
        assert!(matches!(
            ...
            Err(QualificationError::InsufficientEvidence(reason))
                if reason.contains("has no durable activity group")
        ));
    }
    ...
    fn append_unreferenced_source_frames(path: &Path, count: usize) {
        let mut writer = Writer::open(path).unwrap();
        for _ in 0..count {
            writer
                .append(EnvelopeIn {
                    source_id: SourceId("unreferenced".to_owned()),
                    schema_version: 1,
                    parser_version: 1,
                    observed_at: SourceTimestamp(OffsetDateTime::UNIX_EPOCH),
                    received_at: ReceivedAt(OffsetDateTime::UNIX_EPOCH),
                    content_type: ContentType::Json,
                    payload: vec![b' '; 16_384],
                })
                .unwrap();
        }
        writer.sync().unwrap();
    ...
    #[test]
    fn financial_live_posture_retains_only_selected_sources_across_accounts() {
        let temp = tempfile::tempdir().unwrap();
        let (manifest, config) = financial_era_live_source_fixture(temp.path());
        let paths = &manifest.paths;
        let account_a = AccountId::new("live-a").unwrap();
        let account_b = AccountId::new("live-b").unwrap();
        let binding = pe_execution_core::CredentialBindingIdentity {
            version: 1,
            key_id: "key-b".to_owned(),
        };
        let observed_at = OffsetDateTime::from_unix_timestamp(1).unwrap();
        let account_state = authenticated_boundary_state(&account_b, &binding, observed_at);
        let mut baseline = boundary_baseline(&account_b, &binding, &account_state, 1);
        let LiveJournalPayload::AccountPortfolioMarked(mark) = &mut baseline else {
            unreachable!();
        };
        let mut writer = Writer::open(&paths.source_log).unwrap();
        mark.venue_position_evidence =
            append_empty_position_sources(&mut writer, &mark.account_binding, observed_at);
        drop(writer);
        let journal = LiveJournal::open(&paths.live_journal).unwrap();
        journal
            .append(account_b.clone(), observed_at, baseline)
            .unwrap();
        drop(journal);
        // Status names only A; B must be discovered from the journal.
        let live_binding = LiveJournal::verified_tail(&paths.live_journal).unwrap();
        let expected_sources = Reader::replay(&paths.source_log)
            .unwrap()
            .map(|item| item.unwrap().1)
            .collect::<Vec<_>>();
        assert_eq!(expected_sources.len(), 4);
        let expected_keys = expected_sources
            .iter()
            .map(|source| (source.seq, source.this_hash))
            .collect::<HashSet<_>>();
        let expected_bytes = expected_sources
            .iter()
        ...
        for account_id in [account_a, account_b] {
            let events =
                pe_execution_core::replay_account(&paths.live_journal, &account_id).unwrap();
            let selected = crate::live_fanout::projection_source_receipts(&account_id, &events);
            assert_eq!(selected.len(), 2);
            receipts.extend(selected);
        }
        assert_eq!(receipts, expected_keys);

        for unreferenced_frames in [0, 4, 64] {
            append_unreferenced_source_frames(&paths.source_log, unreferenced_frames);
            let prefix = Scanner::inspect(&paths.source_log).unwrap().verified_tail;
            let retained = retain_live_preparation_sources(&prefix, &receipts).unwrap();
            assert_eq!(retained, expected_sources);
            assert_eq!(
                retained
                    .iter()
                    .map(|source| source.payload.len())
            ...
            assert!(retained.windows(2).all(|pair| pair[0].seq < pair[1].seq));
            assert_eq!(
                verify_live_preparation_posture(
                    &paths.live_journal,
                    &paths.source_log,
        ...
        // Neither a matching sequence alone nor a matching hash alone may retain a payload.
        let mismatched_receipts = HashSet::from([
            (EventSeq(0), blake3::hash(b"wrong hash")),
            (EventSeq(4), expected_sources[0].this_hash),
        ]);
        let prefix = Scanner::inspect(&paths.source_log).unwrap().verified_tail;
        assert!(
            retain_live_preparation_sources(&prefix, &mismatched_receipts)
                .unwrap()
                .is_empty()
        );
    }
    ...
    #[test]
    fn financial_live_posture_empty_receipts_still_verify_source_prefix() {
        let temp = tempfile::tempdir().unwrap();
        let source_log = temp.path().join("source.log");
        let live_path = temp.path().join("live.log");
        ...
        let account_id = AccountId::new("live-a").unwrap();
        let events = pe_execution_core::replay_account(&live_path, &account_id).unwrap();
        assert!(events.is_empty());
        let receipts = crate::live_fanout::projection_source_receipts(&account_id, &events);
        assert!(receipts.is_empty());
        let live_binding = LiveJournal::verified_tail(&live_path).unwrap();
        for unreferenced_frames in [0, 4, 64] {
            append_unreferenced_source_frames(&source_log, unreferenced_frames);
            let prefix = Scanner::inspect(&source_log).unwrap().verified_tail;
            assert!(
                retain_live_preparation_sources(&prefix, &receipts)
                    .unwrap()
                    .is_empty()
            );
            assert_eq!(
                verify_live_preparation_posture(&live_path, &source_log, &status_path).unwrap(),
                live_binding
            );
            let mut mismatched = prefix;
            mismatched.last_hash = blake3::hash(b"mismatched binding");
            assert!(matches!(
                retain_live_preparation_sources(&mismatched, &receipts),
                Err(QualificationError::InsufficientEvidence(reason))
                    if reason == "source prefix did not match its verified binding"
            ));
        }
        let (second_offset, _, _) = Reader::replay_with_offsets(&source_log)
            .unwrap()
            .nth(1)
            .unwrap()
            .unwrap();
        append_partial_source_frame(&source_log);
        let scan = Scanner::inspect(&source_log).unwrap();
        assert!(scan.incomplete_tail.is_some());
        assert!(
            retain_live_preparation_sources(&scan.verified_tail, &receipts)
                .unwrap()
                .is_empty()
        );
        let partial_bytes = fs::read(&source_log).unwrap();
        assert_eq!(
            verify_live_preparation_posture(&live_path, &source_log, &status_path).unwrap(),
            live_binding
        );
        assert_eq!(fs::read(&source_log).unwrap(), partial_bytes);
        let mut bytes = fs::read(&source_log).unwrap();
        bytes[usize::try_from(second_offset).unwrap() - 1] ^= 0xff;
        fs::write(&source_log, &bytes).unwrap();
        assert!(matches!(
            verify_live_preparation_posture(&live_path, &source_log, &status_path),
            Err(QualificationError::EventLog(
                pe_event_log::LogError::CrcMismatch { .. }
            ))
        ));
        assert_eq!(fs::read(&source_log).unwrap(), bytes);
    }
    ...
    fn financial_rollback_check_rejects_receipt_missing_from_partial_source_prefix() {
        ...
        let (manifest, config) = financial_era_live_source_fixture(temp.path());
        rollback_check_financial_era(&manifest, &config).unwrap();
        let live_binding = LiveJournal::verified_tail(&manifest.paths.live_journal).unwrap();
        assert_eq!(
            verify_live_preparation_posture(
        ...
        let journal_before = fs::read(&manifest.paths.live_journal).unwrap();
        let account_id = AccountId::new("live-a").unwrap();
        let events =
            pe_execution_core::replay_account(&manifest.paths.live_journal, &account_id).unwrap();
        let LiveJournalPayload::AccountPortfolioMarked(mark) = &events[0].payload else {
            unreachable!();
        };
        assert_eq!(mark.venue_position_evidence.pages.len(), 2);
        let required_receipt = mark.venue_position_evidence.pages[1].receipt;
        ...
        let (offset, missing_sequence, missing_envelope) = &frames[1];
        assert_eq!(
            (*missing_sequence, missing_envelope.this_hash),
            (required_receipt.sequence, required_receipt.this_hash)
        );
        let bytes = fs::read(path).unwrap();
        ...
        assert_eq!(
            Scanner::walk_prefix(&scan.verified_tail, &mut |_, envelope| prefix_sequences
                .push(envelope.seq))
            .unwrap(),
            Some(scan.verified_tail)
        );
        assert_eq!(
            prefix_sequences,
            vec![mark.venue_position_evidence.pages[0].receipt.sequence]
        );
        assert!(!prefix_sequences.contains(missing_sequence));
        let before = fs::read(path).unwrap();

        assert!(matches!(
            verify_live_preparation_posture(&manifest.paths.live_journal, path, &config.status_path),
            Err(QualificationError::InsufficientEvidence(reason))
        ...
        assert_eq!(
            fs::read(&manifest.paths.live_journal).unwrap(),
            journal_before
        );
    }
    ...
    fn commit_read_fixture(
        ...
        .unwrap();
        let payload = crate::bucket_commit::activity_read_commitment_payload_v1(
            continuation.facts.wallet,
            fixed_end,
        ...
        commitment.schema_version =
            crate::bucket_commit::ACTIVITY_READ_COMMITMENT_V1_SCHEMA_VERSION;
        ...
        *continuation = DecisionContinuationV3::new(
            ...
            Some(crate::bucket_commit::ActivityReadCommitmentReceipt::LegacyV1(commitment.receipt)),
        );
        observations.insert(sequence, commitment);
    }

    fn commit_read_fixture_v2(
        continuation: &mut DecisionContinuationV3,
        observations: &mut BTreeMap<u64, SourceObservation>,
    ) {
        let fixed_end = continuation.facts.decision_inputs["fixed_end"]
        ...
        *continuation = DecisionContinuationV3::new(
            ...
            continuation.page_occurrences.clone(),
            Some(
                crate::bucket_commit::ActivityReadCommitmentReceipt::BindingsV2(commitment.receipt),
            ),
        );
        observations.insert(sequence, commitment);
    }
    ...
    fn prefix_at(sequence: Option<u64>) -> TailBinding {
        TailBinding {
            physical_tail: 0,
    ...
    /// FAIL: mutable SQLite proof can redefine a committed complete read while retaining authentic receipts.
    #[test]
    fn read_commitment_rejects_authentic_child_collapse_and_origin_substitution() {
        let (mut continuation, mut observations) = split_source_fixture();
        commit_read_fixture(&mut continuation, &mut observations);
        let temp = tempfile::tempdir().unwrap();
        let state = PaperStateDb::open(&temp.path().join("state.db")).unwrap();
        store_read_decision(&state, &continuation, "decision_pending", true);
        ...
        for changed in [collapsed, substituted] {
            let error = verify_decision_source_inputs(
                &state,
                &replayed_no_copy_decision(&changed),
                &observations,
            )
            .unwrap_err();
    ...
    /// PASS: V3/V4, mixed reads, straddling receipts, and pre-Start websocket membership seal with the exact in-prefix classification; downgrades refuse.
    /// FAIL: service membership disagrees with atomic sealing or erased receipts establish a pre-Start exclusion.
    #[test]
    fn service_and_transactional_sealer_agree_across_read_generations() {
        for generation in [3, 4] {
            let fixture = crate::bucket_commit::continuation_v3_tests::binding_fixture(
            ...
            let mut observations = source_observations(
                &source_path,
                &TailBinding::from(&Scanner::verify(&source_path).unwrap()),
            )
            .unwrap();
            observations.retain(|_, source| {
                source.source_id == crate::trade_poller::ACTIVITY_POLL_SOURCE_ID
            });
            for source in observations.values_mut() {
                source.schema_version = pe_source_polymarket_public::ACTIVITY_SCHEMA_VERSION;
            }
            let connection =
                rusqlite::Connection::open(fixture.dir.path().join("paper.db")).unwrap();
            for row in state.open_decision_pending().unwrap() {
                let original = DecisionContinuationV3::from_durable(&row).unwrap();
                let mut facts = original.facts;
                facts.paper_freshness_policy = None;
                facts.provenance = pe_copy_signal_engine::TradeProvenance::RestPoll;
                let mut continuation =
                    DecisionContinuationV3::new(facts, None, original.page_occurrences, None);
                if generation == 4 {
                    commit_read_fixture(&mut continuation, &mut observations);
                }
                connection.execute(
            ...
            let sealed = prefix_at(observations.last_key_value().map(|(sequence, _)| *sequence));
            let reads = complete_activity_read_scopes(
                &state.decision_pending_history().unwrap(),
                sealed.last_sequence.unwrap(),
                SealedSource::Map(&observations),
            )
            .unwrap();
            assert_eq!(reads.len(), 1);
            assert_eq!(reads[0].decisions.len(), 2);
            assert_eq!(reads[0].continuation.version(), generation);
            let selected = decision_rows_from_source_observations(
                &state,
                &prefix_at(None),
                &sealed,
                &observations,
            ...
            assert_eq!(selected.in_prefix.len(), 2);
            state
            ...
            for pre_start_ws in [false, true] {
                ...
                if pre_start_ws {
                    let payload = serde_json::to_vec(&serde_json::json!({"topic":"activity","type":"trades","payload": {
                        "proxyWallet": continuation.facts.wallet.to_string(), "conditionId":"0xfirst", "asset":"0xfirst",
                        "side":"BUY", "size":1, "price":0.5, "timestamp":100, "transactionHash":"0xfirst", "outcomeIndex":0
                    }})).unwrap();
                    let payload = serde_json::to_vec(
                        &serde_json::from_slice::<serde_json::Value>(&payload).unwrap()["payload"],
                    )
                    .unwrap();
                ...
                store_read_decision(&state, &continuation, "decision_pending", true);
                let sealed = prefix_at(Some(if committed { 3 } else { 2 }));
                let selected = decision_rows_from_source_observations(
                    &state,
                    &prefix_at(Some(1)),
                    &sealed,
                ...
                .unwrap();
                assert_eq!(selected.rows.len(), usize::from(!pre_start_ws));
                assert_eq!(selected.in_prefix.len(), 1);
                let keys = selected
                    .rows
                    .iter()
                    .map(|row| (row.source_trade_id.clone(), row.semantic_revision.clone()))
                ...
                if committed {
                    for version in [2, 3] {
                        ...
                        rusqlite::Connection::open(&path)
                            ...
                            .unwrap();
                        assert!(
                            decision_rows_from_source_observations(
                                &state,
                                &prefix_at(Some(1)),
                                &sealed,
                                &observations
        ...
        for generation in [3, 4, 5] {
            let committed = generation >= 4;
            let (mut continuation, mut observations) =
                single_read_fixture(2, "0xstraddling", "BUY", false);
            let first_payload = full_page_with_target(
                activity_row("0xearlier", "earlier", "1", "0.5", "0xearlier", 100),
                100,
            );
            let first = activity_observation(1, &first_payload);
            let second = observations[&2].clone();
            let pages = [
                activity_page_pair(&first, &first_payload, None, 100, 0),
                activity_page_pair(
                    &second,
                    &second.payload,
                    None,
            ...
            continuation.page_occurrences = pages.iter().map(|(page, _)| page.clone()).collect();
            continuation.facts.decision_inputs["pages"] =
                serde_json::to_value(producer_page_evidence(&pages)).unwrap();
            observations.insert(1, first);
            if committed {
                if generation == 5 {
                    commit_read_fixture_v2(&mut continuation, &mut observations);
                } else {
            ...
            let temp = tempfile::tempdir().unwrap();
            let state = PaperStateDb::open(&temp.path().join("straddling.db")).unwrap();
            store_read_decision(&state, &continuation, "decision_pending", true);
            let sealed = prefix_at(Some(if committed { 3 } else { 2 }));
            let selected = decision_rows_from_source_observations(
                &state,
            ...
            state
                ...
                .unwrap();
            if committed {
                observations.get_mut(&1).unwrap().schema_version =
                    pe_source_polymarket_public::ACTIVITY_SCHEMA_VERSION;
                let error = decision_rows_from_source_observations(
                    &state,
                    &prefix_at(Some(1)),
                    &sealed,
                    &observations,
                ...
                .unwrap_err();
                assert!(error.to_string().contains("page generation differs"));
                assert!(
                    state
                        .seal_decision_evidence_for_source_prefix(&[], &[], sealed.last_sequence)
                        .is_err()
                );
            }
        }
        let (historical, mut observations) = single_read_fixture(1, "0xold", "BUY", false);
        let (committed, newer) = single_read_fixture(2, "0xnew", "BUY", true);
        observations.extend(newer);
        let temp = tempfile::tempdir().unwrap();
        let state = PaperStateDb::open(&temp.path().join("mixed.db")).unwrap();
        store_read_decision(&state, &historical, "decision_pending", true);
        store_read_decision(&state, &committed, "decision_pending", true);
        let selected = decision_rows_from_source_observations(
            &state,
            &prefix_at(None),
            &prefix_at(Some(3)),
    ...
    /// PASS: the source verifier refuses different generations claiming one read; the sealer
    /// still classifies both receipt-bearing rows and refuses an omitted row.
    #[test]
    fn shared_read_generation_disagreement_cannot_hide_a_row() {
        let (mut first, mut observations) = single_read_fixture(2, "first", "BUY", false);
        commit_read_fixture_v2(&mut first, &mut observations);
        let mut facts = first.facts.clone();
        facts.source_trade_id = SourceTradeId(format!("g2:{}", "b".repeat(64)));
        facts.paper_freshness_policy = None;
        let second = DecisionContinuationV3::new(
            facts,
            None,
            first.page_occurrences.clone(),
            first
                .read_commitment
                .map(crate::bucket_commit::ActivityReadCommitmentReceipt::LegacyV1),
        );
        let temp = tempfile::tempdir().unwrap();
        let state = PaperStateDb::open(&temp.path().join("shared.db")).unwrap();
        ...
        assert!(
            complete_activity_read_scopes(&rows, EventSeq(3), SealedSource::Map(&observations))
                .is_err()
        );
        let first_key = (
            first.facts.source_trade_id.clone(),
            first.facts.semantic_revision.clone(),
        );
        let second_key = (
            second.facts.source_trade_id.clone(),
            second.facts.semantic_revision.clone(),
        );
        assert!(
            state
                .seal_decision_evidence_for_source_prefix(
    ...
    /// PASS: a durable post-Start SELL needs no decision, while decision_pending needs terminal receipt-bearing evidence.
    ...
    fn post_start_non_admitted_trade_does_not_block_selection() {
        for disposition in ["not_buy", "decision_pending"] {
            ...
            let temp = tempfile::tempdir().unwrap();
            let state = PaperStateDb::open(&temp.path().join("state.db")).unwrap();
            store_read_decision(&state, &continuation, disposition, false);
            let result = decision_rows_from_source_observations(
                &state,
                &prefix_at(Some(1)),
                &prefix_at(Some(3)),
                &observations,
            ...
            if disposition == "not_buy" {
                ...
                assert!(
                    result
                        .unwrap_err()
                        .to_string()
                        .contains("has no reconstructable receipt-bearing decision")
                );
            }
        }
        for mutation in 0..4 {
            ...
            match mutation {
                0 => {
                    connection.execute("UPDATE decision_pending SET state = 'open', terminal_disposition = NULL", []).unwrap();
                }
                1 => {
                    let mut erased = serde_json::to_value(&continuation).unwrap();
                    ...
                    for key in [
                        ...
                        erased.as_object_mut().unwrap().remove(key);
                    }
                    connection
                        .execute(
                            "UPDATE decision_pending SET frozen_inputs_json = ?1",
    ...
    /// PASS: authentic 4/v1 and 5/v2 selection succeeds; missing, wrong, out-of-prefix,
    /// downgraded, and cross-paired commitments fail over successor pages.
    /// FAIL: erased or substituted commitment evidence enters a sealed selection.
    #[test]
    fn read_commitment_missing_wrong_out_of_prefix_or_downgraded_fails() {
        for version in [4, 5] {
            let (mut continuation, mut observations) =
                single_read_fixture(2, "0xtarget", "BUY", true);
            if version == 5 {
                commit_read_fixture_v2(&mut continuation, &mut observations);
            }
            for mutation in 0..6 {
                ...
                assert!(
                    ...
                    .is_ok(),
                    "authentic generation {version}",
                );
                let mut changed = serde_json::to_value(&continuation).unwrap();
                let mut source = observations.clone();
                match mutation {
                    0 => {
                    ...
                    4 => {
                        source.get_mut(&3).unwrap().source_id =
                            crate::trade_poller::ACTIVITY_POLL_SOURCE_ID.to_owned();
                    }
                    _ => {
                        changed["version"] = serde_json::json!(if version == 5 { 4 } else { 5 });
                        if version == 5 {
                            changed
                                .as_object_mut()
                                .unwrap()
                            ...
                            changed["paper_freshness_policy"] = serde_json::json!({
                                "activity_ws_enabled": false,
                                "copy_latency_budget_secs": 2,
                            });
                        }
                ...
                rusqlite::Connection::open(&path)
                    ...
                    .unwrap();
                let sealed = prefix_at(Some(if mutation == 2 { 2 } else { 3 }));
                assert!(
                    decision_rows_from_source_observations(
                        &state,
                        &prefix_at(Some(1)),
                    ...
                    .is_err(),
                    "generation {version}, mutation {mutation}",
                );
            }
        }
    }
    ...
    /// PASS: both selectors authenticate a bound sibling before transactional sealing, even
    /// when its altered effective identity agrees with a tampered durable correction.
    /// FAIL: an authentic representative lets a sibling's unsupported condition/outcome seal.
    #[test]
    fn shared_read_sibling_identity_is_authenticated_before_selection_and_sealing() {
        for field in ["condition", "outcome"] {
            let mut fixture = crate::bucket_commit::continuation_v3_tests::binding_fixture(
                "shared_recorded_correction",
            );
            let source_path = fixture.dir.path().join("binding.log");
            let aggregate = fixture
                ...
                .into_iter()
                .find(|aggregate| {
                    aggregate.group_id.key() != &fixture.continuation.facts.source_trade_id
                })
                .unwrap();
            let mut commitment: crate::bucket_commit::ActivityReadCommitment =
                serde_json::from_slice(
                    &fixture
                        .index
                        .source_envelope(fixture.continuation.read_commitment.unwrap())
                        .unwrap()
                        .payload,
                )
                .unwrap();
            // Bind the second history leg too; the original fixture's bound leg sorts first.
            let stream_payload = serde_json::to_vec(&serde_json::json!({
                "proxyWallet": fixture.continuation.facts.wallet.to_string(),
                "conditionId": "other", "asset": "456", "side": "BUY", "size": 1,
                "price": 0.5, "timestamp": 99, "transactionHash": "tx", "outcomeIndex": 0
            }))
            .unwrap();
            let stream = parse_activity_trade_observation(&stream_payload).unwrap();
            let metadata = br#"[{"conditionId":"other","clobTokenIds":["456","789"]}]"#.to_vec();
            let metadata_hash =
                pe_source_polymarket_public::canonical_page_hash(&metadata).unwrap();
            let at = OffsetDateTime::from_unix_timestamp(100).unwrap();
            let mut writer = Writer::open(&source_path).unwrap();
            let mut append = |source: &str, schema, parser, payload| {
                writer
                    .append_synced(EnvelopeIn {
            ...
            commitment
                ...
                .push(crate::bucket_commit::ObservationBinding {
                    ...
                    page_raw_hash: fixture.continuation.page_occurrences()[0].raw_hash.clone(),
                    page_occurrence_index: 0,
                    identity_provenance: Some(crate::asset_identity::IdentityProvenance {
                        asset: pe_core_types::PolymarketTokenId("456".to_owned()),
                        source_log_sequence: metadata_receipt.sequence.0,
                        canonical_page_hash: metadata_hash.clone(),
                    }),
                    identity_receipt: Some(metadata_receipt),
                });
            let payload = crate::bucket_commit::activity_read_commitment_payload_v2(
                fixture.continuation.facts.wallet,
                100,
                fixture.continuation.page_occurrences(),
                &serde_json::from_value::<Vec<ReconciliationPageEvidence>>(
                    fixture.continuation.facts.decision_inputs["pages"].clone(),
                )
                .unwrap(),
                commitment.bindings.as_ref().unwrap(),
            )
            .unwrap();
            fixture.continuation.read_commitment = Some(append(
            ...
            drop(writer);
            fixture.index = SourceReceiptIndex::replay(&source_path).unwrap();
            let state = crate::bucket_commit::continuation_v3_tests::binding_state(&fixture);
            for row in state.open_decision_pending().unwrap() {
                state
                    .close_decision_pending(&row.source_trade_id, "{}", "no_fill", 100)
            ...
            assert_eq!(
                &rows[1].source_trade_id, id,
                "bound row must be the sibling"
            );
            let representative = rows[0].clone();
            let candidate = Scanner::verify(&source_path).unwrap();
            let sealed = TailBinding::from(&candidate);
            let start = prefix_at(None);
            let select = |indexed| {
                if indexed {
            ...
            let seal = |selected: SelectedDecisionRows| -> Result<_, QualificationError> {
                assert_eq!(selected.rows.len(), 2);
                assert_eq!(selected.in_prefix.len(), 2);
                let keys = selected
                    .rows
                    ...
                    .map(|row| (row.source_trade_id.clone(), row.semantic_revision.clone()))
                    .collect::<Vec<_>>();
                Ok(state.seal_decision_evidence_for_source_prefix(
                    &keys,
                    &selected.in_prefix,
                    sealed.last_sequence,
                )?)
            };
            let authentic = select(false).and_then(seal).unwrap();
            assert_eq!(authentic, select(true).and_then(seal).unwrap());

            let mut continuation = DecisionContinuationV3::from_durable(&rows[1]).unwrap();
            if field == "condition" {
                continuation.facts.market_id = MarketId(VenueMarketId("unbound".to_owned()));
            } else {
                continuation.facts.outcome_id = OutcomeId(1);
            }
            ...
            verify_decision_continuation_facts(
                &aggregate,
                &continuation.facts,
            ...
            assert_eq!(state.decision_pending_history().unwrap()[0], representative);
            for indexed in [false, true] {
                let result = select(indexed).and_then(seal);
                assert!(
                    result.is_err(),
                    "{field}, indexed={indexed}: tampered sibling reached transactional sealing"
                );
                let error = result.unwrap_err();
                assert!(
                    matches!(&error, QualificationError::InsufficientEvidence(message)
                        if message.contains("binding metadata differs from the effective target identity")),
                    "{field}, indexed={indexed}: {error}"
                );
            }
        }
    }
    ...
    #[test]
    fn continuation_fact_correction_binds_both_identities() {
        let (continuation, observations) = single_read_fixture(2, "0xraw", "BUY", true);
        let aggregate = parsed_aggregates(&[&observations[&2].payload]).remove(0);
        let mut frozen = continuation.facts;
        verify_decision_continuation_facts(&aggregate, &frozen, None).unwrap();
        let stamped = MarketOutcomeId::new(frozen.market_id.clone(), frozen.outcome_id);
        frozen.market_id = MarketId(VenueMarketId("0xverified".to_owned()));
        frozen.outcome_id = OutcomeId(1);
