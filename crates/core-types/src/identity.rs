...
/// Operator identity derived by blake3-hashing the canonical funding-graph root set.
/// Serde: lowercase hex (no 0x prefix). Debug: first 8 hex chars only.
#[derive(Clone, Copy, PartialEq, Eq, Hash)]
...
impl fmt::Debug for OperatorId {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        write!(f, "OperatorId({}…)", &hex.as_str()[..8])
    }
}

impl Serialize for OperatorId {
    fn serialize<S: serde::Serializer>(&self, s: S) -> Result<S::Ok, S::Error> {
        s.serialize_str(self.0.to_hex().as_str())
    }
}
...
/// The root funding wallet that seeded a cluster of trading wallets.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct FunderRootId(pub WalletAddress);

impl fmt::Display for FunderRootId {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::Display::fmt(&self.0, f)
    }
}

impl Serialize for FunderRootId {
    fn serialize<S: serde::Serializer>(&self, s: S) -> Result<S::Ok, S::Error> {
        self.0.serialize(s)
    }
}

impl<'de> Deserialize<'de> for FunderRootId {
    fn deserialize<D: serde::Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
        WalletAddress::deserialize(d).map(FunderRootId)
    }
}
