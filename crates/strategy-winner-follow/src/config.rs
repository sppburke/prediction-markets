...
pub enum SizingMode {
    ...
    /// fraction + price-derived math; the per-trade cap, risk gate, and (step 5c) book cap still
    /// apply. This is the migration target for the legacy `flat_usd_per_trade`.
    Dollar { usd: Decimal },
    /// Fixed contract count, then clamped by the per-trade cap / risk gate / book cap.
