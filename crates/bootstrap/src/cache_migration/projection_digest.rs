...
//! and hashes one sorted-key JSON object per row (#675). The join must read the
//! caller's own transaction, so it stays on the calling thread; building and
//! serializing each row's object moves to workers, and the calling thread feeds
//! the unchanged hasher in query order, so the committed bytes do not change.
...
use super::{BootstrapError, RANKER_PROJECTION_DIGEST_SQL, to_i64};
...
fn spawn<'scope>(
    ...
    workers: usize,
) -> Result<Vec<Worker>, BootstrapError> {
    (0..workers)
        .map(|index| {
        ...
        while !exhausted && read_failure.is_none() && cursor.in_flight < workers.len() * 2 {
            match rows.next() {
                ...
                Ok(Some(row)) => match current.push(row) {
                    ...
                    Ok(()) => {
                        if current.full() {
                            dispatch(workers, cursor, &mut current)?;
                        }
                    }
                },
...
fn dispatch(
    ...
    let batch = mem::replace(current, RawBatch::new());
...
fn take(workers: &[Worker], cursor: &mut Cursor) -> Result<Option<EncodedBatch>, BootstrapError> {
...
fn discard(workers: &[Worker], cursor: &mut Cursor) {
...
mod tests {
    ...
    fn a_column_the_typed_read_rejects_fails_with_the_same_error() {
        let (_dir, connection) = projected_cache(700, 700);
    ...
    fn rows_are_hashed_in_committed_order_not_arrival_order() {
        ...
        let (_dir, connection) = projected_cache(1_300, 1_300);
        let expected = serial_digest(&connection).unwrap();
        connection
            .execute_batch(
