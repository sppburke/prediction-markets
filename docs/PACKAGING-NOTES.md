...
- Added `_GLOSSARY.md`: vocabulary (wallet/trader/operator/leader/candidate), type aliases, resolver-card sub-types with a fully populated example, latency budget (p50/p95/p99), per-venue rate-limit table, anti-gaming flag thresholds, idempotency-key definition (`observed_at_bucket`), promotion/demotion criteria with KS p-value and z-score thresholds, "very liquid market" gate, source freshness defaults, and concrete defaults for previously vague qualifiers (`funding_max_hops`, `fresh_wallet_max_closed_trades`, `cluster_coord_min_members_K`, etc.).
...
- Added `BacktestReport` + `WinnerFollowReport` (extension struct) in `05-` covering turnover, copy-delay percentiles, edge decay by delay bucket, leader churn, demotion-cause histogram, operator-aware exposures, paper-vs-backtest KS p-value, and z-score.
...
Use `18-CODEX-RUST-BOOTSTRAP-PROMPT.md`, `AGENTS.md`, `SKILLS.md`, `_BASELINE.md`, and `_GLOSSARY.md` to bootstrap the Git repository. Build Polymarket paper-copy Winner-Follow before any scaled live trading. Keep inherited-prior first-trade and cluster-coordination modes paper/shadow until they pass their separate promotion ladders in `19-`.
