...
| `clob_best_ask_fallback_haircut_bps` | 100 | Fallback BUY haircut (bps) when a `clob_best_ask` paper fill has no usable best-ask — SELL entry, empty / errored / timed-out `/book`, missing CLOB token, or a zero / zero-size ask. Recorded fill `= leader·(1 + bps/10_000)`, tagged `FillSource::Fallback`, position kept. The haircut demoted from primary to fallback (5%→1%, #486). |
...
| `SUPABASE_FETCH_LIMIT` | 50 | **Module const** in `crates/service/src/supabase_reader.rs`. Top-N wallets fetched per refresh (`?limit=`); equals `MAINTAINED_SET_SIZE` — the refresh fetches exactly the maintained set. The ranker pushes a deeper top-200 bench; only the live set is capped. |
| `MAINTAINED_SET_SIZE` | 50 | **Module const** in `supabase_reader.rs` (issue #350 WS1; replaces `SUPABASE_LIVE_CAP`=200). Fixed size of the maintained live working set. The periodic refresh is score-update-only (no add/evict); membership follows `watchlist_membership_mode`: in `knockout` the maintenance tick (#350 WS1 PR-D) evicts inactive/underperforming wallets and backfills freed slots from the bench up to this width; in `full_rerank` (the cutover production mode) each batch transition replaces the set wholesale at this width. The bench (`latest_ranking`) still holds the ranker's top-200 push; only the live set is capped. |
