...
| `activity_collection_transient_retries` | 16 | **Binary const** `ACTIVITY_COLLECTION_TRANSIENT_RETRIES` in `pe-bootstrap`'s `main.rs` (not a TOML/env key). Overrides `polymarket_max_retries` for `cache-populate-activity-v2` only. One failing page ends the whole collection with exit 75 and cancels every unfinished wallet read (wallets the writer already accepted are kept and skipped on resume), costing the supervisor's 60 s wait, a restart and those unfinished reads. With `bootstrap_polymarket_wallet_timeout_secs` set (#681), a page failure that exhausts these retries no longer ends the collection: that wallet's read is retried in place under its budget and the wallet is excluded when the budget expires. Backoff is 200 ms doubling, capped at 30 s: eight retries wait about 51 s in total and sixteen about 291 s; with the 10 s request timeout a persistent failure exits after at most about eight minutes plus rate-gate waits. Observed on Forge on 2026-09-18 with eight: five exits between 13:18Z and 15:41Z, each on one page 1,000 to 2,500 rows deep in a large wallet ("error sending request"), while the other wallets kept receiving 10 to 15 pages/s; four of the five wallets were then read in full without error right after the restart, and a DNS and TCP-connect probe of the venue host every 5 s saw no failure around the fifth. A stuck page holds only its own wallet slot. The cause, found afterwards, is the venue answering those pages just after the 10 s request timeout (see `activity_collection_request_timeout_secs`); a retry succeeds only once the venue has that answer warm (#588). |
| `activity_collection_request_timeout_secs` | 30 | **Binary const** `ACTIVITY_COLLECTION_REQUEST_TIMEOUT_SECS` in `pe-bootstrap`'s `main.rs` (not a TOML/env key). Overrides `polymarket_request_timeout_secs` for `cache-populate-activity-v2` only; the timeout covers a request from connect to the end of its body. Measured on Forge on 2026-09-18: the two pages of wallet `0x59ee6c6a…` whose failures ended the collection at 16:09Z and 16:34Z answered `200` from a cold venue after 10.4 s and 11.1 s, and after 0.13 s once warm, so each first attempt timed out at 10 s and its retries repeated the cold query. Among 133,747 gaps between consecutive received pages of one wallet (which include retries and rate waits), 246 exceeded 10 s and 47 exceeded 30 s. Thirty seconds is about three times the observed cold answer; with sixteen transient retries a persistent failure exits 75 after at most about 13 minutes plus rate-gate waits (#588). |
| `activity_collection_wallet_fetches` | 32 | **Module const** `MAX_ACTIVITY_WALLET_FETCHES` in `pe-bootstrap`'s `cache_migration.rs` (not a TOML/env key). Wallet reads in flight during `cache-populate-activity-v2`, also the bound of the completed-wallet queue in front of the serial SQLite writer. Each wallet pages serially, so this width sets throughput until another bound binds; it leaves the fetcher's shared rate gate (≤ 20 req/s on `/activity`, above) unchanged. Measured on Forge on 2026-09-18 from committed page timestamps over fixed windows, counted after every wallet active in a window had finished: 16.8 pages/s at 16 (four windows, 16.0–17.6) and 18.3 pages/s at 32 (five windows, 17.6–19.3). Counting sooner undercounts the wider setting more, because each wallet takes longer to finish (#588). |
| `activity_recoverable_retry_secs` | 30 | **Module const** `ACTIVITY_RECOVERABLE_RETRY_SECS` in `pe-bootstrap`'s `cache_migration.rs` (not a TOML/env key). Seconds between in-place retries of a recoverable read failure (transient transport or venue rate limit) under an enabled wallet budget; the venue's own `Retry-After` is honoured when longer. |
| `reconciliation_rate_limit_retry_secs` | 1 | **Crate const** `RECONCILIATION_RATE_LIMIT_RETRY_SECS` in `pe-source-polymarket-public` (not a TOML/env key). HTTP 429 `Retry-After` at or below this is waited out inside `reconciliation_rate_limit_max_retries` by the complete-activity readers only: the service's Data-API reconciliation fetcher and `pe-bootstrap cache-populate-activity-v2` (#588; on Forge every such answer otherwise ended the collection with exit 75, nine times on 2026-09-17, seven of them between 12:01:08Z and 12:44:24Z). Every other fetcher returns 429 to its caller unretried |
| `reconciliation_rate_limit_max_retries` | 10 | **Module const** `RECONCILIATION_RATE_LIMIT_MAX_RETRIES` in `source-polymarket-public::fetcher` (not a TOML/env key). Per-page short-429 retries for opted-in complete-activity readers, independent of `polymarket_max_retries`; neither budget resets or consumes the other. Ten one-second waits span a venue rate window and are cheaper than the approximately two-minute supervised restart reported on Forge (#588). Persistent short 429s return `SourceError::RateLimited` on the eleventh rate-limited response. Every attempt still reserves a rate-gate slot; non-opted-in and long-429 behavior is unchanged. |
...
| `PE_RANK_SCHEMA_TWO_CUTOVER` | `0` | `.env` opt-in read by `scripts/rank_and_push.sh` for lane selection only when a zero-argument production cycle is created, and re-read before any cache activation on every entry to honor `prepare` (#588). `1` enters the schema-two private-candidate lane while the installed cache is still schema one (the one-time initial cutover); `prepare` does the same but stops after exact request preparation (exit 2, which stops the supervisor) with the pending pointer retained, so the full-union collection is measured through the publisher's unchanged freshness checks before any activation; both recovery entries refuse to activate (exit 2) while the value stays `prepare`, and setting it to `1` lets the ordinary pending-publication recovery activate and publish exactly that request. An installed schema-two cache selects the lane regardless of the value. The chosen lane is frozen in the cycle's `cycle_configuration.json` as `cache_lane` (`fresh_v2`; absent means the legacy schema-one contract) and a resumed cycle keeps it. |
...
| `bootstrap_polymarket_pagination` | `"complete-seconds"` | DESC requests bounded by one frozen `hi = now - ACTIVITY_SETTLE_LAG_SECS`; backward `start=1` with complete boundary seconds and durable floor, forward windows scanned upward from the durable frontier and sized by raw density. Each completed piece/window commits rows and outward coverage together, including empty pieces. No known-ID early stop. The full terminal offset page makes a window saturated; a saturated second fails incomplete. See `docs/26` for coverage and accepted limits. |
| `ACTIVITY_SETTLE_LAG_SECS` | 120 | Bootstrap module constant, not configuration. Every new request covers only normalized timestamps at least this far behind the walk's sampled Unix time. The bucket at `hi` has been closed one second less than this lag. Inherited legacy coverage and rows visible only after their crossing request are outside the discipline. Measured rationale: `29-ACTIVITY-LATENCY-MEASUREMENT.md`. |
| `FORWARD_WINDOW_FIRST_SECS` | 1 | First forward-window width on every walk, including resumed walks. A dense first second may require multiple page responses before any durable progress. |
| `FORWARD_WINDOW_TARGET_ROWS` | 2_000 | Target raw response rows, not a bound: after a completed width `w` with `n>0` raw rows, next width is `max(1,w*target/n)`, capped at `8*w` and `FORWARD_WINDOW_MAX_SECS`. An empty window grows eightfold; saturation shrinks width eightfold, with a floor of one second. |
| `FORWARD_WINDOW_MAX_SECS` | 31_536_000 | Maximum forward-window width. Window responses remain bounded by the existing inclusive offset ceiling, independently of width; a saturated attempt is retried narrower. |
...
| `bootstrap_polymarket_wallet_timeout_secs` | 300 | Per-wallet wall-clock budget for `walk_wallet` and, in the schema-two collector, for each wallet's acquisition — recoverable read failures are retried in place under it and expiry records the wallet's exclusion for the generation (#681); `0` disables the timeout. On timeout or error, every committed backward piece and forward window survives; the unfinished window is discarded. `backfill_partial=1` remains due regardless of stamp while active and non-infrastructure, and excluded from schema-one production Python ranking/new publication. `CommitProgress` lives outside the timed future and logs actual durable transactions/rows. Finalization clears the marker with the optional success stamp atomically; failure preserves the old stamp. Set via `PE_BOOTSTRAP_POLYMARKET_WALLET_TIMEOUT_SECS`. |
| `bootstrap_wallet_cache_path` | `"wallet_cache.db"` | SQLite cache. WAL retains committed backward pieces and forward windows; only in-flight transactions and unfinished window buffers are lost on interruption. Set via `PE_BOOTSTRAP_CACHE_PATH`. |
| `bootstrap_cache_page_cache_mib` | 4096 | Operator-selected SQLite page-cache budget in MiB on every writable `WalletCache` open. `BootstrapConfig.cache_page_cache_mib`; set via `PE_BOOTSTRAP_CACHE_PAGE_CACHE_MIB`. Valid range `1..=i32::MAX / 1024` MiB; converted to negative KiB for `PRAGMA cache_size`. Invalid values fail before lock/open/create. No automatic sizing. |
| `bootstrap_cache_mmap_mib` | 2047 | Operator-selected SQLite mmap ceiling in MiB on every writable `WalletCache` open; **0 disables mmap**. `BootstrapConfig.cache_mmap_mib`; set via `PE_BOOTSTRAP_CACHE_MMAP_MIB`. Bytes must fit `i64`. Default is the largest whole-MiB value below bundled SQLite 3.46's supported-platform `SQLITE_MAX_MMAP_SIZE` (`0x7fff0000` bytes); larger requests are valid but clamped. Unavailable mmap has effective ceiling 0. Requested/effective pragmas are logged from the writer connection. Read-only and migration raw connections are unchanged. |
...
| `activity_scan_workers` | `min(3, cores - 1)`, at least 1 | Module const `MAX_WORKERS` in `cache_migration::aggregate_scan`. Decode-and-serialize workers beside the reading/consuming thread, each holding at most two batches. Three was the measured knee on four cores; a two-core host runs one (#670). |
...
| `ranker_classifier_version` | 4 | **Module const** `RANKER_CLASSIFIER_VERSION` in `bootstrap::cache_migration`. Generation of the derived `ranker_entries_v2` first-entry projection rebuilt from retained complete activity. Version 4 takes the live copy path's entries: every bought market consumes its history, as each live position anchor records it; an entry whose action depends on its second's order is not projected; a trade or redemption is classified by its asset's position in its market's payout evidence (`tokens_json`), and an asset missing there is raw-only; after a non-combo redemption that needs a position anchor, nothing is projected for `ranker_redeem_pause_secs` and any later position mutation touching the redeemed condition stops the wallet; another such redemption there restarts the pause (#690). Distinct from `CACHE_SCHEMA_VERSION_V2` (cache schema) and `FINAL_STAGE_RECORD_VERSION` (finalization receipt format), both unchanged. Newly finalized/installed candidates require the current classifier; hash-bound historical caches retain their authentic classifier generation (1 to 4 accepted), with projection rows required to match their certified state. |
| `ranker_redeem_pause_secs` | 604_800 (7 days) | **Module const** `RANKER_REDEEM_PAUSE_SECS` in `bootstrap::cache_migration`. The historical projection admits no entry within this many seconds after a redemption that needs a position anchor: the owner-accepted stand-in for live's wait for that anchor, which history cannot show. It exceeded every completed live pause measured on 2026-09-24 (290, longest 598,774 s); it neither certifies a live anchor nor bounds a future pause (#690). |
...
| `bootstrap_backfill_staleness_secs` | 86_400 (1 day) | Active, non-infrastructure wallets are due when `backfill_partial=1 OR last_polymarket_fetch_at IS NULL OR last_polymarket_fetch_at < now - this`. `pe_bootstrap::pile::BACKFILL_STALENESS_SECS` also supplies purge rule B's freshness gate, which additionally requires marker zero. This governs a completed wallet's next walk, independently of the loop's success wait. |
...
accrues no trades. Rank/export/publication apply trade-recency and current-eligibility
filters; production Python ranking and newly prepared publication additionally exclude all schema-one
partial backfills. They do not consult `is_infra` or `purged_wallets`
