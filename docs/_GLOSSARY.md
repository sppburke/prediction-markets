...
| `demotion_pnl_window_secs` | 2_592_000 | `ServiceConfig` field (item 3.1 of the 2026-07-01 decision record on issue #417). Trailing window (seconds; 30 d) for the demotion realized-P&L conjunct: `WalletEdgeStats::should_demote` requires the dollar P&L over fills **settled** within this window to be negative, so a large historical winner carries no unbounded bleed allowance. Lifetime `realized_pnl` remains an audit field. `PE_DEMOTION_PNL_WINDOW_SECS`. |
