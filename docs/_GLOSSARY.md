...
| `bootstrap_polymarket_pagination` | `"complete-seconds"` | Fixed-end DESC `/activity?type=TRADE` pages. Full-history backward requests use `start=1`; each page boundary second is acquired completely before stepping below it. Forward windows use inclusive on-wire `start=lo+1`, offset pagination, and ordered lower/seam/upper splitting at saturation; #608 buffers the entire forward phase for one commit. No known-ID early stop. The per-window buffer includes the terminal page at `activity_max_offset`; a saturated single second fails incomplete. See `docs/26` for resume and API-visibility scope. |
...
| `bootstrap_polymarket_wallet_timeout_secs` | 300 | Per-wallet wall-clock budget (seconds) for `pe-bootstrap backfill` / `run`'s Polymarket fetch loop (issue #173). `0` disables the timeout; positive values wrap each `fetch_wallet_incremental` call in `tokio::time::timeout`. Wallets that trip the budget are soft-failed (`FetchOutcome::failed`); completed backward pages survive, the uncommitted forward phase is discarded, and `backfill_partial=1` excludes their schema-one history from ranking. The post-fetch pipeline still runs. `last_polymarket_fetch_at` is unchanged (NULL or its old value), so due wallets stay due. Success clears the marker regardless of stamping; optional stamp and marker clear share a transaction. Set via `PE_BOOTSTRAP_POLYMARKET_WALLET_TIMEOUT_SECS`. |
| `bootstrap_wallet_cache_path` | `"wallet_cache.db"` | SQLite trade cache. WAL mode retains committed pages; only in-flight transactions and buffered acquisition are lost on crash. Set via `PE_BOOTSTRAP_CACHE_PATH`. |
