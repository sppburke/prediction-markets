...
| `ranker_fdr_q` | 0.05 | Family significance / false-coverage-rate target for the #421 bake-off harness honesty layer (`docs/31-RANKER-BAKEOFF-METHODOLOGY.md`). The single strategy-level statistical level shared by `fcr_selected_ci(q=…)`, the `select_winner_or_nogo` winner gate's `alpha`, the Romano-Wolf StepM `size`, Hansen-SPA, and the AKM/MRSW post-selection inference in `scripts/ranker/{oos_validation,bakeoff}.py`. Bootstrap reps/seeds and CSCV group counts are method-internal parameters, **not** glossary'd (see `oos_validation.py` header). |
