...
| `crypto_shadow_flush_interval_ms` | 250 | Frame-buffer flush cadence in `drive` (issue #311): buffered raw frames + trade prints are written in ONE batched transaction every this-many ms, or as soon as `crypto_shadow_flush_max_frames` are buffered, whichever first (plus once at loop exit). Replaces the per-frame auto-commit (~450–1800 ms/s of blocking at ~900 frames/s — the v2 saturation cause) with ~10–20 ms/s. Crash-loss ≤ one flush window. |
