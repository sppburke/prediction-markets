...
| `rank_and_push_tempfail_exit` | 75 | Exit code used when a bounded retryable rank-cycle operation exhausts its in-process retries or the resolution audit is blocked (`blocked > 0 \|\| clipped > 0` — a market whose available venue truth could not be recorded, or a repair-cap clip; venue-side incompleteness such as lagged/extended/inactive/pending markets is counted and retried without blocking, issue #523). Emitters are Gamma `events` page exhaustion, a blocked resolution audit, and ranking publication. The production loop preserves and resumes the applicable durable logical-cycle or exact-publication pointer; permanent failures remain non-75 and stop the loop. |
