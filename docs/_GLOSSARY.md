...
| `bootstrap_event_orphan_warn_pct` | 5 | `pe-bootstrap events` coverage gate (issue #206): if more than 5% of distinct traded markets fail to map to a Gamma `/events` event (i.e. end up self-mapped as singleton events), log a warning to investigate event coverage. Const `ORPHAN_WARN_PCT` in `pe_bootstrap::events`. |
| `bootstrap_event_orphan_fail_pct` | 50 | `pe-bootstrap events` coverage gate (issue #206): if more than 50% of distinct traded markets fail to map to a Gamma event, abort the sweep loudly — that orphan rate indicates a `conditionId` join-key/form break, not a legitimate unmappable tail. Const `ORPHAN_FAIL_PCT` in `pe_bootstrap::events`. |
...
| `skill_forward_min_bucket_trades` | 5 | Minimum ≤cutoff resolved buys in an entry-price bucket for a reliable calibration `p`; below this the Kelly position falls back to flat-$1 (flagged). Set via `PE_SKILL_FORWARD_MIN_BUCKET_TRADES`. |
