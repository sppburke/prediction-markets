...
| `market_fee_missing_default_bps` | 0 | Default fee in basis points when Gamma omits `takerBaseFee`/`makerBaseFee`. Zero is the safe sentinel: it never over-discounts PnL and treats pre-fee-era markets (≤ 2026-03-30) correctly. Used by `fee_to_bps` in `pe_bootstrap::events`. |
| `market_fee_fraction_threshold` | 1.0 | Detection boundary for Gamma fee encoding: values ≤ 1.0 are treated as fractions (e.g. `0.02` = 2%) and multiplied by 10 000 to obtain bps; values > 1.0 are treated as already-bps integers (e.g. `200`). The threshold is at 1.0 because a fee expressed as 100% (= 1.0 fraction = 10 000 bps) is the maximum meaningful value; any integer bps value > 1 is unambiguously > 1.0. |
| `market_fee_max_bps` | 10_000 | Maximum fee in basis points stored in `market_fees`. All incoming Gamma fee values are clamped to `[0, 10_000]` before insertion. Polymarket's published schedule is ≤ 400 bps (≤ 4%); the 10 000 bps ceiling (100%) is a hard upper guard against malformed API responses. |
