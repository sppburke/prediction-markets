...
| `PE_RANK_SCHEMA_TWO_CUTOVER` | `0` | `.env` opt-in read by `scripts/rank_and_push.sh` only when a zero-argument production cycle is created (#588). `1` enters the schema-two private-candidate lane while the installed cache is still schema one (the one-time initial cutover); `prepare` does the same but stops after exact request preparation with the pending pointer retained, so the full-union collection is measured through the publisher's unchanged freshness checks before any activation; the ordinary pending-publication recovery then activates and publishes exactly that request. An installed schema-two cache selects the lane regardless of the value. The chosen lane is frozen in the cycle's `cycle_configuration.json` as `cache_lane` (`fresh_v2`; absent means the legacy schema-one contract) and a resumed cycle keeps it. |
