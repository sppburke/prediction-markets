...
| `copy_latency_budget_secs` | 2 | #530/#546: while websocket-primary, an observation from EITHER source (REST poll or activity websocket) older than this is admitted with a typed no-copy disposition (`no_copy_dispositions`: `stale_fallback_past_copy_budget` / `stale_activity_ws_past_copy_budget`) and stages no copy — strict `age > budget`, checked at the early admission gate and again immediately before dispatch staging so channel or gate delay cannot produce a stale copy. Matches the deployed ranker `LATENCY_SHIFT_SECS`; re-checked at +1 week against the measured span artifact |
...
`position_validation_current` is not a reuse prerequisite. A wallet whose venue activity history
holds a `TRADE` row with a price outside the unit range (issue #594; observed 2026-09-11) is
excluded from the boot universe like a transient read failure (left unvalidated for runtime
admission; a previously installed anchor stays as it is) and, once live, the periodic anchor refresh
reports it deferred instead of ending the poll round. Malformed pages, window invalidations, other
row validations, aggregation and identity failures remain boot-fatal.
