...
| `activity_collection_transient_retries` | 8 | **Binary const** `ACTIVITY_COLLECTION_TRANSIENT_RETRIES` in `pe-bootstrap`'s `main.rs` (not a TOML/env key). Overrides `polymarket_max_retries` for `cache-populate-activity-v2` only. One failing page ends the whole collection with exit 75 and cancels every unfinished wallet read (wallets the writer already accepted are kept and skipped on resume). The default three retries at 200 ms doubling give about 1.4 s of cumulative backoff; eight give about 51 s, and with the 10 s request timeout a persistent failure exits for the supervised retry after about 141 s plus rate-gate waits. Observed on Forge 2026-09-18: three connection-level "error sending request" exits in the bulk root's first 80 minutes (#588). Eight is an initial tuning choice, to be revisited against observed outage durations. |
