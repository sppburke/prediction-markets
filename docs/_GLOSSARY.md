...
| `ranker_fdr_q` | 0.05 | Family significance / false-coverage-rate target for the #421 bake-off harness honesty layer (`docs/31-RANKER-BAKEOFF-METHODOLOGY.md`). The canonical strategy-level significance level — applied as the default `q`/`alpha`/`size` (the literal `0.05` in code, not yet a single wired config key) by `fcr_selected_ci(q=…)`, the `select_winner_or_nogo` winner gate, the Romano-Wolf StepM, Hansen-SPA, and the AKM/MRSW post-selection inference in `scripts/ranker/{oos_validation,bakeoff}.py`. Bootstrap reps/seeds and CSCV group counts are method-internal parameters, **not** glossary'd (see `oos_validation.py` header). |
| `ranker_pbo_max` | 0.5 | Probability-of-Backtest-Overfitting ceiling in the #421 winner gate (`select_winner_or_nogo`, `scripts/ranker/bakeoff.py`): a config can only be declared WINNER when its CSCV `PBO < 0.5` (the IS-best config lands above the OOS median more often than not). |
| `ranker_dsr_min` | 0.5 | Per-step Deflated-Sharpe survival gate (`apply_deflation_gate` threshold, `scripts/ranker/bakeoff.py`): within a trajectory step, candidate configs with deflated-Sharpe probability `< 0.5` are dropped before the policy acts. Distinct from `ranker_pbo_max` (a leaderboard-level overfit ceiling) though both are 0.5. |
| `ranker_eb_prior_var_floor` | 1e-9 | Variance floor for the empirical-Bayes prior in the `eb_shrinkage_skill` estimator (`scripts/ranker/estimators.py`): `tau2 = max(prior_var, 1e-9)`. The EB prior mean μ0 and variance τ² are **estimated from the cross-section** each scoring step (unlike the fixed `kelly_p_prior_*` Beta priors in `19-WINNER-FOLLOW-STRATEGY.md`); the floor only keeps a degenerate <2-candidate input deterministic. |
...
| `clob_page_max_retries` | 5 | Walk-level retries on a transient / rate-limited CLOB `/markets?closed=true` page fetch before the closed-markets walk aborts (issue #429 follow-up). Sits *on top of* `ReqwestFetcher`'s internal fast retries (`polymarket_max_retries`); rides through *sustained* flakiness (e.g. a minute of `error decoding response body`) so one bad page does not abort the ~1,457-page re-walk. `Fatal` (4xx) errors still abort immediately; the error propagates after the budget is exhausted. Const `CLOB_PAGE_MAX_RETRIES` in `pe_bootstrap::clob`. |
| `clob_page_retry_base_ms` | 1_000 | Base backoff (ms) for the CLOB page retry; exponential (`base · 2^attempt`) capped at 30s. With `clob_page_max_retries=5` the walk rides ~31s of sustained flakiness per page before giving up; `RateLimited` instead honours the server's `retry_after`. Const `CLOB_PAGE_RETRY_BASE_MS` in `pe_bootstrap::clob`. |
