...
| `ranking_publish_tempfail_exit` | 75 | Exit code used when a retryable ranking-publication failure exhausts its bounded in-process retries. The production loop preserves and replays the exact durable publication request; permanent failures remain non-75 and stop the loop. |
| `ranking_publish_loop_retry_secs` | 60 | Delay between loop-level retries of a durable pending ranking publication. The loop checks its run flag once per second during this delay so an operator stop remains prompt. |
