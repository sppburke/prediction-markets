...
| `activity_collection_transient_retries` | 8 | **Binary const** `ACTIVITY_COLLECTION_TRANSIENT_RETRIES` in `pe-bootstrap`'s `main.rs` (not a TOML/env key). Overrides `polymarket_max_retries` for `cache-populate-activity-v2` only. One failing page ends the whole collection with exit 75 and discards every in-flight wallet, while the default three retries at 200 ms doubling cover about 1.4 s; eight cover about 51 s before exiting for the supervised retry. Measured on Forge 2026-09-18: two connection-level "error sending request" exits in 72 minutes of the bulk root (#588). |
