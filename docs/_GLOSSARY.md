...
| `reconciliation_rate_limit_retry_secs` | 1 | **Crate const** `RECONCILIATION_RATE_LIMIT_RETRY_SECS` in `pe-source-polymarket-public` (not a TOML/env key). HTTP 429 `Retry-After` at or below this is waited out inside `polymarket_max_retries` by the complete-activity readers only: the service's Data-API reconciliation fetcher and `pe-bootstrap cache-populate-activity-v2` (#588; on Forge every such answer otherwise ended the collection with exit 75, seven times in 69 minutes on 2026-09-17). Every other fetcher returns 429 to its caller unretried |
