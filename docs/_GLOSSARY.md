...
| `activity_collection_wallet_fetches` | 32 | **Module const** `MAX_ACTIVITY_WALLET_FETCHES` in `pe-bootstrap`'s `cache_migration.rs` (not a TOML/env key). Wallet reads in flight during `cache-populate-activity-v2`, also the bound of the completed-wallet queue in front of the serial SQLite writer. Each wallet pages serially, so this width sets throughput until the fetcher's shared rate gate (≤ 20 req/s on `/activity`, above) binds; it leaves that gate's budget unchanged. At 16 the Forge bulk root committed 13.7 pages/s, an estimated page cycle of about 1.16 s per slot against an uncached venue response of about 0.4 s (2026-09-18, #588). |
