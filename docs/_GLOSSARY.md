...
| `activity_ws_normalized_activity_timeout_secs` | 30 | #546: a reader with no normalizer-accepted activity row for this long is not live — it drops its socket and re-dials after its own backoff (1, 2, 4 … 60 s, reset only by a normalized row). Acknowledgements, keepalives, unrelated topics, envelope errors, and parser-rejected payloads never refresh it. Readiness: 0 live readers or a poisoned sink ⇒ `activity_ws_unavailable`; exactly 1 ⇒ `activity_ws_redundancy_degraded`; unavailable + unhealthy poll ⇒ `copy_admission_blocked` |
