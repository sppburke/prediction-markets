...
| `crypto_shadow_channel_capacity` | 4096 | Bounded `mpsc` capacity for WS frames → join loop. Backpressure: drop-newest on full (dropped frames are logged; under sustained overload `raw_ticks` is best-effort, not gap-free). Raised 1024→4096 in v2: the added CLOB trade tape (`last_trade_price`) roughly doubles CLOB frame volume and a deeper buffer absorbs bursty book+trade activity around a BTC move. |
| `crypto_shadow_clob_subscribe_channel_capacity` | 16 | Bounded `mpsc` capacity for incremental CLOB subscribe commands (`drive`'s refresh arm → CLOB task). One slot per refresh-batch of newly-enumerated tokens. A dropped command is self-healing: every reconnect re-subscribes the full cumulative token set. |
