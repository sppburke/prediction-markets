...
use crate::{
    ...
    derive_proxy_wallet, derive_safe_wallet, ResponseObserver,
};
...
/// Compute the exact EIP-712 hash signed for a standard Polygon V2 order.
///
/// This narrow helper exposes the SDK's real `Order` SolStruct/type name and domain so callers do
/// not risk reconstructing a different EIP-712 type with the same fields.
#[must_use]
pub fn standard_v2_order_hash(order: &crate::clob::types::OrderV2) -> Option<B256> {
    let verifying_contract = contract_config(POLYGON, false)?.exchange_v2?;
    ...
    Some(order.eip712_signing_hash(&domain))
}
...
pub struct Config {
    ...
    /// Optional raw-response observer. Response headers are restricted and request secrets are
    /// never exposed.
...
impl fmt::Debug for Config {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            ...
            .field("geoblock_host", &self.geoblock_host)
...
impl<S: State> Client<S> {
    async fn observed_request<Response: DeserializeOwned>(
        &self,
        request: Request,
        headers: Option<HeaderMap>,
    ) -> Result<Response> {
        match self.inner.config.response_observer.clone() {
            Some(observer) => {
                crate::request_observed(&self.inner.client, request, headers, observer).await
            }
            None => crate::request(&self.inner.client, request, headers).await,
        }
    }
    ...
    pub async fn ok(&self) -> Result<String> {
        ...
        self.observed_request(request, None).await
    }
    ...
    pub(crate) async fn resolve_version(&self, force: bool) -> Result<u32> {
        ...
        let request = self
            ...
            .build()?;
        let body: VersionBody = self.observed_request(request, None).await?;
        self.inner
            .cached_version
    ...
    pub async fn check_geoblock_raw(
        &self,
...
impl<K: Kind> Client<Authenticated<K>> {
    ...
    pub async fn closed_only_mode(&self) -> Result<BanStatusResponse> {
        ...
        self.observed_request(request, Some(headers)).await
    }

    pub async fn closed_only_mode_raw(
        &self,
        deadline: Option<std::time::Instant>,
    ) -> Result<crate::ResponseObservation> {
        let request = self
    ...
    /// Authenticates and submits the exact serialized order body once.
    ///
    /// This method performs no transaction-hash polling, version-mismatch refresh, or response
    /// parsing. The caller receives the exact response for durable capture before parsing.
    ...
    /// Cancels one order once and returns the exact response before status handling or parsing.
    pub async fn cancel_order_once_raw(
        &self,
        order_id: &str,
    ) -> Result<crate::ResponseObservation> {
        let request = self
    ...
    pub async fn balance_allowance_raw(
        &self,
        mut request: BalanceAllowanceRequest,
        deadline: Option<std::time::Instant>,
    ) -> Result<crate::ResponseObservation> {
