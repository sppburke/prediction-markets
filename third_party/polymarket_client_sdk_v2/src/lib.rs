...
/// Captured non-secret response metadata and exact response bytes.
...
/// Exact request identity and timing for a transport failure with no fabricated response body.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct TransportFailureObservation {
    pub method: String,
    pub path: String,
    pub ordered_query: Vec<(String, String)>,
    pub observed_at: SystemTime,
    pub received_at: SystemTime,
    pub error_class: TransportFailureClass,
}

impl std::fmt::Display for TransportFailureObservation {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{:?} making {} request to {}", self.error_class, self.method, self.path)
    }
}

impl std::error::Error for TransportFailureObservation {}

/// Synchronous response callback for callers that must persist evidence before parsing.
...
async fn request<Response: DeserializeOwned>(
    ...
    request_inner(client, request, headers, None).await
}
...
async fn request_observed<Response: DeserializeOwned>(
    client: &reqwest::Client,
    request: Request,
    headers: Option<HeaderMap>,
    observer: Arc<dyn ResponseObserver>,
) -> Result<Response> {
    request_inner(client, request, headers, Some(observer)).await
}
...
async fn request_raw_once(
    ...
    let remaining = match deadline {
        Some(deadline) => Some(
            deadline
                ...
                .ok_or_else(|| Error::validation("request deadline elapsed before send"))?,
        ),
        None => None,
    };
            ...
            .map_err(|_| {
                Error::with_source(
                    ...
                    TransportFailureObservation {
                        ...
                        error_class: TransportFailureClass::Timeout,
                    },
                )
            })?
    } else {
        ...
        Error::with_source(
            ...
            TransportFailureObservation {
                ...
                error_class: if error.is_timeout() {
                    ...
                    TransportFailureClass::Other
                },
            },
        )
    })?;
    let status = response.status().as_u16();
    let headers = ["content-type", "date", "etag", "retry-after", "x-request-id"]
        .into_iter()
        .filter_map(|name| {
            response
                .headers()
    ...
    let body = if let Some(deadline) = deadline {
        ...
        tokio::time::timeout(remaining, body_read)
            .await
            .map_err(|_| {
                Error::with_source(
                    crate::error::Kind::Internal,
    ...
    .map_err(|_| {
        Error::with_source(
            ...
            TransportFailureObservation {
                ...
                observed_at,
                received_at: SystemTime::now(),
                error_class: TransportFailureClass::BodyRead,
            },
        )
    })?
    .to_vec();
...
async fn request_inner<Response: DeserializeOwned>(
    client: &reqwest::Client,
    mut request: Request,
    headers: Option<HeaderMap>,
    ...
    let observed_headers = ["content-type", "date", "etag", "retry-after", "x-request-id"]
        .into_iter()
        .filter_map(|name| {
            response
                .headers()
    ...
    if let Some(observer) = observer {
        observer.observe(ResponseObservation {
